...
def zwaveEvent(physicalgraph.zwave.commands.notificationv3.NotificationReport cmd) {
	...
	if (cmd.notificationType == 0x07) {
		if (cmd.event == 0x01 || cmd.event == 0x02) {
			...
			result << createEvent(descriptionText: "$device.displayName has unknown command")		 
		}
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv2.WakeUpNotification cmd) {
	def event = createEvent(name: "WakeUp", value: "wakeup", descriptionText: "${device.displayName} woke up", isStateChange: true)  // for Astralink
	def cmds = []

	if (!isConfigured() || !device.currentState("ManufacturerCode")) {
...
def zwaveEvent(physicalgraph.zwave.commands.manufacturerspecificv2.ManufacturerSpecificReport cmd) {
	...
	if (manufacturerCode == "0109" && productTypeCode == "2002") {
		...
		result << secure(zwave.configurationV1.configurationSet(parameterNumber: 1, configurationValue: [1], size: 1))
		setConfigured(true)
	}
...
private setConfigured(value) {
	updateDataValue("configured", value.toString())
}

private isConfigured() {
	getDataValue("configured").toString() == "true"
}